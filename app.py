...
STOCKS = {
    'Nabil Bank':                  {'close': 'models/model_nabil.pkl',       'high': 'models/model_nabil_high.pkl',       'low': 'models/model_nabil_low.pkl',       'pro': False},
    'Nepal Doorsanchar 🔒':        {'close': 'models/model_doorsanchar.pkl', 'high': 'models/model_doorsanchar_high.pkl', 'low': 'models/model_doorsanchar_low.pkl', 'pro': True},
...
PRO_PRICE    = "NPR 59 / month"
CODES_FILE   = "pro_codes.txt"   # file in your GitHub repo — one code per line, USED:CODE or UNUSED:CODE
...
def _gh_put(filename, content, sha, message):
    ...
    if sha:
        payload["sha"] = sha
    requests.put(url, headers={**_gh_headers(), "Content-Type": "application/json"},
                 data=json.dumps(payload))

# ── One-time burn code logic ──
def verify_and_burn_code(entered):
    ...
    for line in lines:
        ...
        if not line:
            continue
        if ":" in line:
            status, code = line.split(":", 1)
            status = status.strip().upper()
            code   = code.strip().upper()
        else:
            # plain code with no prefix = unused
            status, code = "UNUSED", line.upper()

        if code == entered:
            found = True
            if status == "USED":
                already   = True
                new_lines.append(f"USED:{code}")
            else:
                # burn it
                new_lines.append(f"USED:{code}")
        else:
            new_lines.append(f"{status}:{code}")

    if not found:
        return False, "❌ Invalid code. Please check and try again."
    if already:
        return False, "❌ This code has already been used. Each code is valid for one device only."

    # save burned code back to GitHub
    _gh_put(CODES_FILE, "\n".join(new_lines) + "\n", sha, f"burn code: {entered}")
    return True, "✅ Pro access unlocked!"
...
<div style="display:flex; gap:12px; margin-bottom:1.5rem;">
    <div style="flex:1; background:linear-gradient(145deg,rgba(56,189,248,0.08),rgba(99,102,241,0.1));
        border:1px solid rgba(56,189,248,0.3); border-radius:14px; padding:1rem; text-align:center;">
        <div style="font-family:'Orbitron',sans-serif; font-size:0.75rem; color:#38BDF8; letter-spacing:2px;">FREE</div>
        <div style="font-size:1.4rem; font-weight:800; color:#F1F5F9; margin:0.3rem 0;">NPR 0</div>
        <div style="font-size:0.78rem; color:#94A3B8;">✅ Nabil Bank predictions<br>✅ BUY/SELL signals<br>✅ Unlimited use</div>
    </div>
    <div style="flex:1; background:linear-gradient(145deg,rgba(245,158,11,0.1),rgba(239,68,68,0.08));
        border:1px solid rgba(245,158,11,0.4); border-radius:14px; padding:1rem; text-align:center;">
        <div style="font-family:'Orbitron',sans-serif; font-size:0.75rem; color:#F59E0B; letter-spacing:2px;">⚡ PRO</div>
        <div style="font-size:1.4rem; font-weight:800; color:#F59E0B; margin:0.3rem 0;">NPR 59<span style="font-size:0.8rem;color:#94A3B8;">/mo</span></div>
        <div style="font-size:0.78rem; color:#94A3B8;">✅ Everything in Free<br>🔒 NTC + CIT predictions<br>🔒 More stocks soon</div>
    </div>
</div>
""", unsafe_allow_html=True)
...
# ── PRO PAYWALL ──
if info['pro'] and not st.session_state.pro_unlocked:
    clean_name = stock_name.replace(' 🔒', '')
    st.markdown(f"""
    <div style="background:linear-gradient(145deg,rgba(245,158,11,0.08),rgba(239,68,68,0.08));
        border:1px solid rgba(245,158,11,0.4); border-radius:20px; padding:2rem; text-align:center; margin:1.5rem 0;">
        <div style="font-family:'Orbitron',sans-serif; font-size:1.3rem; font-weight:800;
            background:linear-gradient(90deg,#F59E0B,#EF4444); -webkit-background-clip:text;
            -webkit-text-fill-color:transparent; margin-bottom:0.5rem;">⚡ PRO PLAN REQUIRED</div>
        <p style="color:#94A3B8; font-size:0.9rem; margin:0.5rem 0 0.8rem;">
            <b style="color:#F8FAFC;">{clean_name}</b> is available for Pro members only.
        </p>
        <div style="font-size:2rem; font-weight:800; color:#F59E0B; margin:0.8rem 0;">{PRO_PRICE}</div>
        <div style="font-size:0.85rem; color:#CBD5E1; margin:0.3rem 0;">✅ Unlock Nepal Doorsanchar (NTC)</div>
        <div style="font-size:0.85rem; color:#CBD5E1; margin:0.3rem 0;">✅ Unlock Citizen Investment Trust (CIT)</div>
        <div style="font-size:0.85rem; color:#CBD5E1; margin:0.3rem 0;">✅ More stocks coming soon</div>
        <div style="font-size:0.85rem; color:#CBD5E1; margin:0.3rem 0;">✅ Priority support</div>
        <br>
        <p style="color:#64748B; font-size:0.78rem;">
            📩 Pay &amp; get your one-time access code:<br>
            <b style="color:#38BDF8;">smartpriceai@gmail.com</b> &nbsp;|&nbsp;
            WhatsApp: <b style="color:#38BDF8;">+977-9805357571</b>
        </p>
        <p style="color:#475569; font-size:0.72rem; margin-top:0.4rem;">
            🔐 Each code is <b style="color:#F59E0B;">one-time use only</b> — valid for this session.<br>
            🎁 Early subscribers get <b style="color:#F59E0B;">3 months free!</b>
        </p>
    </div>
    """, unsafe_allow_html=True)

    st.markdown('<div class="section-label">Enter your one-time access code</div>', unsafe_allow_html=True)
    col_code, col_btn = st.columns([3, 1])
    with col_code:
        entered_code = st.text_input("code", placeholder="e.g. SP-A1B2C3",
                                     label_visibility="collapsed", max_chars=20)
    with col_btn:
        verify = st.button("Unlock →", use_container_width=True)
    ...
    if verify:
        if entered_code.strip() == "":
            ...
            with st.spinner("Verifying..."):
                success, msg = verify_and_burn_code(entered_code)
            if success:
...
if st.session_state.pro_unlocked:
    st.markdown("""
    <div style="background:linear-gradient(135deg,rgba(245,158,11,0.12),rgba(99,102,241,0.1));
        border:1px solid rgba(245,158,11,0.5); border-radius:12px; padding:0.6rem 1rem;
        text-align:center; color:#F59E0B; font-weight:700; font-size:0.85rem; margin-bottom:1rem;">
        ⚡ PRO ACCESS ACTIVE — All stocks unlocked
    </div>
    """, unsafe_allow_html=True)
